...
try {
    ...
    if (false === mb_strpos($incPath, $addPath)) {
    ...
    if (empty($_REQUEST['xoopsUrl']) && !$origin) {
        ...
        if (($origin && !isset($allowOrigins[$origin]))
         || (isset($_SERVER['HTTP_ACCESS_CONTROL_REQUEST_METHOD'])
                 && !in_array(mb_strtoupper($_SERVER['HTTP_ACCESS_CONTROL_REQUEST_METHOD']), ['POST', 'GET', 'OPTIOINS']))
        ) {
    ...
    $netVolumeData = null;
    if ($userRoll['uid'] && null === $session->get('netvolume')) {
    ...
    if (isset($_SESSION['XELFINDER_RF_' . $mydirname]) && $_SESSION['XELFINDER_CFG_HASH_' . $mydirname] === $config_MD5) {
        ...
        } elseif (empty($config['disable_pathinfo'])) {

...
class xoops_elFinder
{
    ...
    public function __construct($mydirname, $opt = [])
    {
        ...
        $this->base64encodeSessionData = ((!defined('_CHARSET') || _CHARSET !== 'UTF-8') && 'blob' !== mb_substr($this->getSessionTableType(), -4));
    ...
    public function checkLogin($session)
    {
        ...
        if (isset($_GET['login']) || isset($_GET['logout']) || isset($_GET['status'])) {
            ...
            exit();
        } elseif (!empty($_GET[_MD_XELFINDER_PROXY_TOKEN_KEY])) {
            $this->tokenDataGC();
            // check token
        ...
        foreach ($configs as $_conf) {
            ...
            if ($options) {
                ...
                foreach ($options as $_op) {
                    if (0 === mb_strpos($_op, 'gid=')) {
                        $_gids = array_map('intval', explode(',', mb_substr($_op, 4)));
                        if ($_gids && $this->mygids) {
                            if (!array_intersect($this->mygids, $_gids)) {
                                continue 2;
                            }
                        }
                    } elseif (0 === mb_strpos($_op, 'defaults=')) {
                        list(, $_tmp) = explode('=', $_op, 2);
                        $defaults = $this->defaultVolumeOptions['defaults'];
                        $_tmp = mb_strtolower($_tmp);
                        foreach ($defaults as $_p) {
                            if (false !== mb_strpos($_tmp, $_p[0])) {
                                $defaults[$_p] = true;
                            }
                        }
                    } elseif (0 === mb_strpos($_op, 'plugin.')) {
                        list($_p, $_tmp) = explode('=', mb_substr($_op, 7), 2);
                                ...
                                switch (mb_strtolower($_v)) {
                                    ...
                                    default:
                                        ...
                                        if ('`' === $_fc && '`' === $_lc) {
                                            ...
                                            is_numeric($_v) && ($_v = mb_strpos($_v, '.') ? (float)$_v : (int)$_v);
                                        }
                                }
                                $_opts[trim($_k)] = $_v;
                            }
    ...
    public function netmountPreCallback()
    {
        ...
        if (mb_strlen(serialize($_SESSION)) > 64000) {
...
EOD;
    ...
    {
        if ($this->file && ($fp = @fopen($this->file, 'ab'))) {
            fwrite($fp, $log . "\n");
            fclose($fp);
        }
    }
    ...
    public static function getUnameByUid($uid)
    {
        ...
        if (0 === $uid) {
            ...
            $xoopsConfig = $config_handler->getConfigsByCat(XOOPS_CONF);
            $uname = $xoopsConfig['anonymous'];
            if ('utf8' === self::$dbCharset && 'UTF-8' !== mb_strtoupper(_CHARSET)) {
                $uname = mb_convert_encoding($uname, 'UTF-8', _CHARSET);
            }
        } else {

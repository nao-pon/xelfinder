...
class elFinderVolumeXoopsXelfinder_db extends elFinderVolumeDriver
{
        ...
        if ($this->nameAccepted($name)) {
            ...
            if (is_dir($localpath)) {
                ...
                if ($check_mime_accept) {
                    ...
                    if ('allow' == mb_strtolower($this->uploadOrder[0])) {
    ...
    protected function makeStat($stat)
    {
        ...
        $name = $stat['name'];
        if ('directory' !== $stat['mime']) {
            if (false === mb_strpos($this->options['URL'], '?')) {
    ...
    protected function cacheDir($path)
    {
        ...
        if (!empty($current_stat['filter'])) {
            ...
            $filter = mb_substr($filter, 1, mb_strlen($filter) - 2);
            $filter = str_replace('*', '%', $filter);
            $q = [];
            foreach (explode(' ', $filter) as $val) {
                if (false === mb_strpos($val, '/') && false === mb_strpos($val, '%')) {
                    $val .= '%';
                }
                $q[] = '`mime` LIKE \'' . $val . '\'';
            }
        ...
        if ($mimes) {
            ...
            foreach ($mimes as $mime) {
                if (false === mb_strpos($mime, '/')) {
                    ...
                    $mime = mb_substr($mime, 1, mb_strlen($mime) - 2);
    ...
    protected function _stat($path, $rootCheck = true)
    {
        ...
        } elseif ($rootCheck && $path == $this->root) {
    ...
    protected function readlink($path, $make = false)
    {
        ...
        if (empty($stat['_localalias'])) {
            ...
            $link = $stat['alias'];
            if ('/' === mb_substr($link, 1, 1)) {
                $_head = mb_substr($link, 0, 1);
                if (false !== mb_strpos($link, '%')) {
                    $link = dirname($link) . DIRECTORY_SEPARATOR . rawurldecode(basename($link));
                }
                switch ($_head) {
                    case 'R':
                        $link = XOOPS_ROOT_PATH . mb_substr($link, 1);
                        break;
                    case 'T':
                        $link = XOOPS_TRUST_PATH . mb_substr($link, 1);
                        break;
                }
            }
        }
    ...
    protected function _copy($source, $targetDir, $name)
    {
        ...
        $sql = $id > 0
            ? sprintf(
                'REPLACE INTO %s (`file_id`, `parent_id`, `name`, `size`, `ctime`, `mtime`, `mime`, `width`, `height`, `gids`, `uid`, `gid`, `perm`, `umask`, `mime_filter`)
			                    (SELECT  %d,        %d,         `name`, `size`, `ctime`, `mtime`, `mime`, `width`, `height`, `gids`, `uid`,  %d,   "%s",   "%s"   , `mime_filter`  FROM %s WHERE file_id=%d)',
                $this->tbf,
                (int)$id,
                (int)$this->_dirname($id),
                $gid,
                $perm,
                $umask,
                $this->tbf,
            ...
            : sprintf(
                'INSERT INTO %s (            `parent_id`, `name`, `size`, `ctime`, `mtime`, `mime`, `width`, `height`, `gids`, `uid`, `gid`, `perm`, `umask`, `mime_filter`)
        ...
        if ($this->query($sql)) {
            ...
            if ($local = $this->readlink($id, true)) {
                if ($target = @fopen($local, 'wb')) {
                    ...
                    fclose($target);
                    if ('internal' === $this->mimeDetect && 0 === mb_strpos($mime, 'image') && !getimagesize($local)) {
                        $this->_unlink($id);

                        return $this->setError(elFinder::ERROR_UPLOAD_FILE_MIME);
                    }
                    if ($this->options['autoResize'] && 0 === mb_strpos($mime, 'image') && max($w, $h) > $this->options['autoResize']) {
    ...
    protected function _filePutContents($path, $content)
    {
        if ($local = $this->readlink($path)) {
            if (false !== file_put_contents($local, $content)) {
                ...
                return $this->query(sprintf('UPDATE %s SET `size`=%d, `mtime`=%d, `mime`="%s" WHERE `file_id` = "%d" LIMIT 1', $this->tbf, mb_strlen($content), $time, $mime, $path));
            }
        }
